...
import SharePostGroup from '../group/SharePostGroup'
...
const PostFeed = ({value, group}) => {
    ...
    return(
       ...
       <div style={{display: 'flex', flexDirection: 'column', marginLeft: '10px', justifyContent: 'center'}}>
           ...
           <Link to={`/group/${group._id}`}><Text fontWeight='600'>{group.name}</Text></Link>
           </div>
       ...
       <div className='post-group-page-post'>
           ...
           <Like id={value._id} option={`group/${value.group}/post`} liked={value.liked} number={value.like}/>
           <Link to={`/group/${value.group}/p/${value._id}`}><FaComment color='#aab8c2'/></Link>
           <SharePostGroup id={value._id} group={value.group}/>
           </div>
       </div>
   </article>
    )
