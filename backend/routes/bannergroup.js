...
router.post('/:id/banner' , verify, admin, upload.single('image'), async(req, res) => {
...
try{
    ...
    await Group.updateOne({_id: group}, {banner: name})
        return res.json({path: `bannergroup/${name}`})
}catch(err) {
    res.status(500).send()
}})
